...
3. **此 repo（jgu4）** — `cluster.yaml` 填值 → `task configure --yes` → commit & push
...
```sh
kubectl --kubeconfig ~/coding/jgu4/kubeconfig-sa <command>
# or
export KUBECONFIG=~/coding/jgu4/kubeconfig-sa
```

- `kubeconfig` requires OIDC browser auth — cannot be used non-interactively.
- `kubeconfig-sa` has an embedded Omni SA token (SA: `claude-code`, Admin, valid ~1 year from 2026-05-09).
